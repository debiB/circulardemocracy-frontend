 ...
 */
function substituteVariables(text: string, data: { name?: string; campaign?: string }): string {
  let result = text;
      ...
      <CardContent className="space-y-4">
        ...
        {(personalizationData.name || personalizationData.campaign) && (
          <div className="bg-blue-50 border border-blue-200 rounded p-3">
            <div className="text-xs font-medium text-blue-800 mb-2">Personalization Applied:</div>
            <div className="space-y-1 text-xs text-blue-700">
              {personalizationData.name && (
                <div>• Name: <span className="font-medium">{personalizationData.name}</span></div>
              )}
              {personalizationData.campaign && (
                <div>• Campaign: <span className="font-medium">{personalizationData.campaign}</span></div>
              )}
            </div>
          </div>
        )}
        ...
        {!personalizationData.name && !personalizationData.campaign && (
          <div className="bg-yellow-50 border border-yellow-200 rounded p-3">
            <div className="text-xs text-yellow-800">
              No personalization data provided. Variables like {'{name}'} and {'{campaign}'} will show as placeholders.
            </div>
          </div>
        )}
      </CardContent>
